...
import { parsePostgREST } from './parsePostgREST';
...
describe('valueSource: "value"', () => {
  ...
  it('handles negated operations as the only rules', () => {
    expect(parsePostgREST({ '!': { '<=': [12, { var: 'f1' }, 14] } })).toEqual({
      combinator: 'and',
      rules: [{ field: 'f1', operator: 'notBetween', value: '12,14' }],
    });
    expect(parsePostgREST({ '!': { in: [{ var: 'f1' }, [12, 14]] } })).toEqual({
      combinator: 'and',
      rules: [{ field: 'f1', operator: 'notIn', value: '12,14' }],
    });
    expect(parsePostgREST({ '!': { startsWith: [{ var: 'f1' }, 'Test'] } })).toEqual({
      combinator: 'and',
      rules: [{ field: 'f1', operator: 'doesNotBeginWith', value: 'Test' }],
    });
    expect(parsePostgREST({ '!': { endsWith: [{ var: 'f1' }, 'Test'] } })).toEqual({
      combinator: 'and',
      rules: [{ field: 'f1', operator: 'doesNotEndWith', value: 'Test' }],
    });
    expect(parsePostgREST({ '!': { in: [{ var: 'f1' }, 'Test'] } })).toEqual({
      combinator: 'and',
  ...
  it('handles negated groups', () => {
    expect(parsePostgREST({ '!': { and: [{ '==': [{ var: 'f1' }, 'Test'] }] } })).toEqual({
      combinator: 'and',
      not: true,
      rules: [{ field: 'f1', operator: '=', value: 'Test' }],
    });
...
it('parses JsonLogic as string', () => {
  expect(parsePostgREST('{"and":[{"==":[{"var": "f1"},12]},{"==":[{"var": "f2"},14]}]}')).toEqual({
    combinator: 'and',
    rules: [
      { field: 'f1', operator: '=', value: 12 },
...
it('validates fields', () => {
  ...
  expect(parsePostgREST(jsonLogicForFields, { getValueSources, fields })).toEqual(
    ruleGroupForFields
  );
  expect(
    parsePostgREST(jsonLogicForFields, {
      getValueSources,
...
it('invalidates primitives as root object', () => {
  ...
  expect(parsePostgREST(true)).toEqual(emptyRuleGroup);
  expect(parsePostgREST(1214)).toEqual(emptyRuleGroup);
  expect(parsePostgREST('Test')).toEqual(emptyRuleGroup);
});
...
it('parses custom operations', () => {
  ...
  expect(
    parsePostgREST({ regex: [{ var: 'f1' }, /test/] } as unknown as RQBJsonLogic, {
      jsonLogicOperations,
    })
  ).toEqual({
    combinator: 'and',
...
it('parses custom group operations', () => {
  ...
  for (const [op, result] of customGroupOpTests) {
    expect(
      parsePostgREST({ [op]: [] } as RQBJsonLogic, { jsonLogicOperations: { [op]: () => result } })
    ).toEqual(
      result
        ? isRuleGroup(result)
          ? result
...
it('handles empty options object', () => {
  expect(parsePostgREST({ '===': [{ var: 'f1' }, 1] }, {})).toEqual({
    combinator: 'and',
